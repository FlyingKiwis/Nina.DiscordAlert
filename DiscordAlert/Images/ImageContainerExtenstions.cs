...
namespace NINA.DiscordAlert.Images {
    public static class ImageContainerExtenstions
    {
        public static string FillPlaceholders(this ISavedImageContainer image, string text) 
        {
            Logger.Debug($"Replacing placeholders.  Image={image?.PathToImage?.ToString() ?? "none"}");
            try {
                var patterns = GetImagePatterns(image);
                return patterns.GetImageFileString(text);
            } catch (Exception ex) {
                Logger.Error($"Error occured while replacing text {nameof(text)}={text}", ex);
                return text;
            }
        }

        //This function is copied and slightly modified from NINA itself, I could not find a way that it was exposed to plugins 
        public static ImagePatterns GetImagePatterns(this ISavedImageContainer image) {
